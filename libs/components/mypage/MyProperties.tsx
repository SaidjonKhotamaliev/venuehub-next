...
const MyProperties: NextPage = ({ initialInput, ...props }: any) => {
	...
	const deletePropertyHandler = async (id: string) => {
		try {
			if (await sweetConfirmAlert('Are you sure that you want to delete this property?')) {
				await updateProperty({
					variables: {
						input: { _id: id, propertyStatus: 'DELETE' },
					},
				});
	...
	const updatePropertyHandler = async (status: string, id: string) => {
		try {
			if (await sweetConfirmAlert(`Are you sure to change to ${status} status?`)) {
				await updateProperty({
					variables: {
						input: { _id: id, propertyStatus: status },
					},
				});
				await getAgentPropertiesRefetch({ input: searchFilter });
			}
	...
	if (device === 'mobile') {
		return <div>VENUEHUB PROPERTIES MOBILE</div>;
	} else {
		return (
			<div id="my-property-page">
				<Stack className="main-title-box">
					<Stack className="right-box">
						<Typography className="main-title">My Venues</Typography>
